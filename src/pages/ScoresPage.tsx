...
export const ScoresPage = () => {
  const { state, addScore, resetScores } = useAppState();
  ...
  return (
    <section>
      ...
      <div className="grid-form">
        ...
        <button className="secondary-btn" onClick={resetScores}>Reset score history</button>
      </div>
      {state.scoreHistory.length === 0 ? <p>No scores saved yet.</p> : <ul>{state.scoreHistory.map((s) => <li key={s.date + s.overallPct}>{s.date}: P {s.propertyPct}% | C {s.casualtyPct}% | O {s.overallPct}%</li>)}</ul>}
    </section>
  );
};

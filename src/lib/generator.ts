import { Card, QuestionBankItem, Settings, Topic, STUDY_TOPICS } from './types';
...
export const DEFAULT_QUIZ_BANK: QuestionBankItem[] = [
...
export const generateCards = (config: GeneratorConfig, _userState: UserState, count: number, seed = Date.now()): Card[] => {
  ...
  const bank = config.questionBank.filter((q) => allowedTopics.includes(q.topic));
